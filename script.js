...
document.querySelector(".nav-active").addEventListener("click", (e) => {
...
document.querySelector(".x").addEventListener("click", (e) => {
  ...
  document.querySelector(".x").classList.add("none");
});
...
// let arr = s.split("");
// /*
// find the longest substring using s (input)
// return length of substring !!!substring cant be a sequence
//
...
// while(i == arr.length){
// i++
//  if(unique(arr.substring(arr[pointer],i)){
// greatest++
// }else{
// pointer = i
// if(greatest > answer) answer = greatest
// greatest = 0
// }
// }
// "pwwkew"
...
// index++;
// console.log(unique(s.substring(pointer, index)));
// console.log(s.substring(pointer, index));
// console.log(pointer);

// if (unique(s.substring(pointer, index))) {
// answer = Math.max(answer, s.substring(pointer, index).length);
// } else {
// pointer++;
// index = pointer;
// }
...
let next = null;
for (let i in input) {
  current = next || +input[+i + 1] + +input[+i + 2] + +input[+i];
  next = +input[+i + 2] + +input[+i + 1] + +input[+i + 3];
...
let aim = 0;
let depth = 0;
input1.forEach((current) => {
  let [position, val] = current.split(" ");
  if (position == "forward") {
    forward += +val;
    depth += +val * aim;
  }
